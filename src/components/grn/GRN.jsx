...
export default function GRN() {
  ...
  const [data, requestPO] = useApi(
    "https://67c168b561d8935867e2e089.mockapi.io/api/poMaster/purchaseOrder"
  );
  const [grnData, requestGRN] = useApi(
    "https://67c6b64f351c081993fe6412.mockapi.io/grnMaster"
  );
  ...
  useEffect(() => {
    let id = localStorage.getItem("GRNEditId");
  ...
  const vendorSelection = async (id) => {
    const res = await requestPO("GET");
    console.log(res, "dfsdfsdf");
    let po = res.filter((ele) => {
  ...
  const POSelection = async (e, id) => {
    if (e.target.checked) {
      console.log(id, "id for addining");
      let selectedPo;
      ...
      console.log(selectedPo, "selected po");

      const updated = grnForm;
      ...
      const updatedItems = await Promise.all(
        selectedPo.itemDetails
          .filter((ele) => ele.itemStatus === "pending")
          .map(async (ele) => {
            const prom = await useCalGrn(
              grnForm.Vendor,
              ele.po_id,
              ele.itemDetail_id,
              ele.grnQty
            );
            console.log(prom, ele.grnQty, "grn Qty");

            return {
              ...ele,
              grnQty: 0,
              preGrnQty: parseFloat(prom).toFixed(3),
            };
          })
      );

      console.log(updatedItems, "from po updated item");
      ...
      setItems((prev) => prev.filter((ele) => ele.po_id !== id));

      let up = grnForm;
      const updatedPurchaseOrder = up.PurchaseOrder.filter(
        (po) => po.po_id !== id
      );
      up.PurchaseOrder = [...updatedPurchaseOrder];
      up.items = up.items.filter((ele) => {
        return ele.po_id !== id;
      });
      console.log(up, "skdfhskdh");
      setGrnForm((prev) => {
        return { ...up };
      });
    }
  ...
  const itemSelection = (e, itemId, index) => {
    let updatedItem = grnForm;

    if (e.target.checked) {
      console.log(items, "from itemselection");
      let tempItem = items[index];
      tempItem.isSelected = true;
      // console.log("tmpitem======", tempItem);

      updatedItem.items = [...updatedItem.items, tempItem];

      setGrnForm((prev) => {
        return { ...updatedItem };
      });
    } else {
      updatedItem.items = updatedItem.items.filter((ele) => {
  ...
  async function save() {
    ...
    if (editingID) {
      let editGrn = grnData.find((ele) => {
        return ele.GRN_id === editingID;
      });
      console.log(editGrn, editGrn.id, editingID, grnForm);
      requestGRN("PUT", grnForm, editGrn.id);
      setEditingID((prev) => {
        return "";
      });
    } else {
      console.log("grnForm=============================>", grnForm);
      requestGRN("POST", grnForm);
    }
    ...
    const res = await requestGRN("GET");
    const res2 = await requestPO("GET");
  ...
  async function DeleteGrn() {
    ...
    requestGRN("DELETE", null, editGrn.id);
  ...
  async function handleEdit(idx) {
    let id = Number(idx);
    let grnList = await requestGRN("GET");
    setEditingID(id);

    localStorage.setItem("GRNEditId", id);
    const purchaseOrder = await requestPO("GET");
    let up = grnList.find((ele) => {
      return ele.GRN_id === id;
    });
    let poItem = [];
    console.log(up, "editing up");

    poItem = up.PurchaseOrder.map((po) => {
      const foundItem = purchaseOrder.find((ele) => ele.po_id === po.po_id);
      if (foundItem) {
        foundItem.isSelected = true;
        return foundItem;
      }
      return null;
    }).filter(Boolean); // Removes null values if no match was found

    console.log(poItem);

    poItem.forEach((ele) => {
      console.log(ele);
      ele.itemDetails.forEach((it) => {
        up.items.map((i) => {
          if (i.itemDetail_id == it.itemDetail_id && i.po_id == it.po_id) {
            it.grnQty = i.grnQty;
    ...
    console.log(poItem, "Active PO");

    setActivePO((prev) => {
      return poItem;
    });

    let upItem = await Promise.all(
      up.items.map(async (ele) => {
        const prom = await useCalGrn(
          up.Vendor,
          ele.po_id,
          ele.itemDetail_id,
    ...
    console.log(upItem, "from editing");
  ...
  const handleBlur = (e, index, itemId) => {
    ...
    if (name === "DocNo") {
      setGrnForm((prev) => {
        return {
          ...prev,
          DocNo: "GRN" + String(value.slice(3)).padStart(8, "0"),
        };
      });
    }
    if (name === "grnQty") {
