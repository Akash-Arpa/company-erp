...
function MaterialIssue() {
  ...
  function PopulateEdit(updatedLocalData) {
    ...
    const materialIssues =
      JSON.parse(localStorage.getItem("MaterialIssue")) || [];
    ...
    if (issue) {
      ...
      const newData = updatedLocalData.map((item) => {
        const existingItem = issue.issuedMaterial.find(
          (issuedItem) => issuedItem.item_id == item.item_id
        );
        if (existingItem) {
  ...
  const saveItemData = () => {
    setLocalData((prevData) => {
      ...
      let existingItem = updatedData.find(
        (item) => item.item_id == modalData[0].item_id
      );
  ...
  const itemMarked = (id, status) => {
    ...
    if (status.checked) {
      ...
      if (item.lots.length > 0)
        alert("Issue quantity of this item if present will be cleared");
      item.lots = [];
      item.item_quantity = "";
    }
  };
      ...
      const updatedData = await Promise.all(
        localData.map(async (prev) => {
          ...
          let updatedLots = data.lots.map((lot) => {
            let matchedMaterial = prev.lots.find(
              (material) => material.item_id === lot.item_id
            );
            return matchedMaterial
              ? { ...lot, issueQty: matchedMaterial.issueQty }
              : lot;
          });
      ...
      if (!valid) {
        ...
        if (!valid) {
          ...
          if (Number(quantity) != Number(total)) {
            alert(
              "There is a disparity between the quantity entered and the total quantity issued"
            );
            return;
          } else {
            let MaterialIssue =
            ...
            const issueToEdit = sessionStorage.getItem("issueToEdit");
            if (issueToEdit) {
              const index = MaterialIssue.findIndex(
                (issue) => issue.docNo == issueToEdit
              );
              if (index !== -1) {
              ...
              if (AuthorizedState) {
                alert(
                  "Any edit was not permited therefore the Material Issue stays as it is"
                );
      ...
      <table>
        <tbody>
          <tr>
            <td>Doc No.</td>
            <td>
              <input id="docNo" type="text" readOnly={AuthorizedState} />
            </td>
          </tr>
          <tr>
            <td>Doc Date</td>
            <td>
              <input
                type="date"
                onInput={(e) => updateCalender(e.target.value)}
                id="docDate"
          ...
          <tr>
            <td>Issue To</td>
            <td>
              <input type="text" id="issuedTo" readOnly={AuthorizedState} />
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td>
              <button onClick={() => saveMaterialIssue()}>Save</button>
            </td>
            <td>
              <button onClick={() => deleteMaterialIssue()}>Delete</button>
            </td>
            <td>
              <button onClick={handleOpenDashboard}>Open</button>
            </td>
          </tr>
          <tr>
            <td colSpan="2">
                      ...
                      <tr key={item.item_id}>
                        <td>
                          <input
                            ...
                            onClick={(e) => itemMarked(item.item_id, e.target)}
                        ...
                        <td>
                          <button onClick={() => populateItem(item.item_id)}>
                            Populate
                          </button>
                        </td>
                      </tr>
